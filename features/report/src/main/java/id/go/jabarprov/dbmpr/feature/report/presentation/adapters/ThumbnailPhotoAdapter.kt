...
class ThumbnailPhotoAdapter(private val onClickListener: () -> Unit) :
    ListAdapter<PhotoModel, ThumbnailPhotoAdapter.PhotoItemViewHolder>(PhotoModelDiffUtils()) {
    ...
    override fun onBindViewHolder(holder: PhotoItemViewHolder, position: Int) {
        getItem(position)?.let {
            holder.bind(it, hiddenItem, onClickListener)
        }
    }
    ...
    class PhotoItemViewHolder(private val binding: LayoutThumbnailSmallBinding) :
        ...
        fun bind(photo: PhotoModel, hiddenItem: Int, action: () -> Unit) {
            binding.apply {
